...
def sysadmin_courses(
    sysadmin: models.SystemAdmin = Depends(get_curr_admin), # Returns Student model
    db: Session = Depends(get_db)
    ):
    all_courses = crud.get_all_courses(db,skip=0,limit=20)
...
def sysadmin_home(
    sysadmin: models.SystemAdmin = Depends(get_curr_admin), # Returns systemadmin model
    db: Session = Depends(get_db)
    ):
...
def sysadmin_course_view(
    course_id: int,
    db: Session = Depends(get_db),
    sysadmin: models.SystemAdmin = Depends(get_curr_admin)
):
    course = db.query(models.Course).filter(models.Course.course_id == course_id).first()
