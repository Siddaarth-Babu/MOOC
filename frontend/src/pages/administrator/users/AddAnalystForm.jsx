...
const AddAnalystForm = ({ onCreated = () => {}, onCancel = () => {} }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    setError('')
    if (!form.name || !form.email_id || !form.password) {
    ...
    try {
      ...
      const res = await fetch(`${BASE}/admin/data_analyst/new_data_analyst`, {
        method: 'POST',
        credentials: 'include',
      ...
      console.error('create analyst', err)
  ...
  return (
    <form onSubmit={handleSubmit} className="add-user-form" style={{ marginTop: 12 }}>
      <h3 style={{ marginTop: 0 }}>Add Data Analyst</h3>
      ...
      <div style={{ marginTop: 10 }}>
        <button className="auth-submit-btn" type="submit" disabled={loading}>{loading ? 'Creating...' : 'Create Analyst'}</button>
